...
import { MoreVertical, UserPlus, Trash2, X, ArrowLeft, Lock, Mail, ShieldAlert, ShieldCheck } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [managerEmail, setManagerEmail] = useState(''); // NEW: Manager Portal State
  ...
  // NEW: Extracted 'role' and 'promoteToManager'
  const { user, role, signInWithMagicLink, signOut, fetchProfile, promoteToManager } = useAuthStore();
            ...
            <div className="flex flex-col gap-2 divide-y divide-slate-100">
              {roommates.map(r => (
                <div key={r.id} className="flex justify-between items-center py-4 first:pt-0">
                  ...
                  <button onClick={() => handleDeleteAttempt(r.id, r.name)} className="px-5 py-2.5 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 font-black rounded-xl flex items-center gap-2 transition-colors active:scale-95 shadow-sm border border-red-100">
  ...
  return (
    <main className="min-h-screen p-4 md:p-8 max-w-7xl mx-auto flex flex-col gap-6 pb-24 relative overflow-x-hidden animate-in fade-in duration-300">
      ...
      <header className="flex justify-between items-center pt-4 relative z-50 min-h-[60px]">
        ...
        {/* NEW: Only Managers see the 3-dot menu */}
        {activeTab === 'entries' && user && role === 'manager' && (
          <div className="relative">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-3 rounded-2xl bg-white/50 backdrop-blur-xl border border-white/60 shadow-sm hover:bg-white/80 transition-all active:scale-95 text-slate-700">
            ...
            {isMenuOpen && (
              <div className="absolute right-0 top-14 w-80 bg-white/90 backdrop-blur-3xl border border-white shadow-2xl rounded-3xl p-5 flex flex-col gap-4 origin-top-right animate-in fade-in zoom-in-95 duration-200 z-50">
                ...
                {/* NEW: Assign a Manager Form */}
                <form onSubmit={handleAssignManager} className="flex gap-2">
                  <input type="email" placeholder="Assign new manager email..." value={managerEmail} onChange={(e) => setManagerEmail(e.target.value)} className="flex-1 bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500 placeholder-emerald-600/50" />
                ...
                <form onSubmit={handleAddMember} className="flex gap-2">
                  <input type="text" placeholder="New member name..." value={newMemberName} onChange={(e) => setNewMemberName(e.target.value)} className="flex-1 bg-slate-100/50 border border-slate-200 rounded-xl p-3 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500" />
                ...
                <div className="border-t border-slate-200 pt-4 mt-1">
                  <button onClick={() => { setIsMenuOpen(false); setShowDeletePage(true); }} className="w-full py-3 bg-red-50 hover:bg-red-100 text-red-600 border border-red-100 font-black tracking-wide rounded-xl flex items-center justify-center gap-2 transition-colors active:scale-95 shadow-sm mb-2"><Trash2 className="w-5 h-5" /> Remove Members</button>
                  <button onClick={() => { setIsMenuOpen(false); signOut(); }} className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-black tracking-wide rounded-xl flex items-center justify-center gap-2 transition-colors active:scale-95 shadow-sm">Log Out</button>
                </div>
                      ...
                      todaysPayments.map(p => {
                        ...
                        return (
                          <div key={p.id} className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-100 shadow-sm">
                            <div className="flex flex-col"><span className="text-xs font-bold text-slate-700 truncate max-w-[150px]">{p.description}</span><span className="text-[9px] font-black text-slate-500 uppercase tracking-wider">{member?.name} ৳ {Number(p.amount).toFixed(2)}</span></div>
      ...
      {activeTab === 'ledger' && (
        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-6 rounded-squircle bg-white/40 backdrop-blur-xl border border-white/50 shadow-md flex flex-col justify-center items-center"><p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Month Cost</p><p className="text-4xl font-black text-slate-800">৳ {totalCost.toFixed(2)}</p></div>
            <div className="p-6 rounded-squircle bg-white/40 backdrop-blur-xl border border-white/50 shadow-md flex flex-col justify-center items-center"><p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Meals Logged</p><p className="text-4xl font-black text-slate-800">{totalMeals}</p></div>
            <div className="p-6 rounded-squircle bg-white/40 backdrop-blur-xl border border-white/50 shadow-md flex flex-col justify-center items-center"><p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Current Meal Rate</p><p className="text-4xl font-black text-blue-600">৳ {mealRate.toFixed(2)}</p></div>
          </div>
          ...
          <div className="p-6 md:p-8 rounded-3xl bg-white/60 backdrop-blur-2xl border border-white/80 shadow-xl overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead><tr className="border-b-2 border-slate-200 text-slate-500 text-xs font-black uppercase tracking-wider"><th className="pb-4 min-w-[120px]">Member Entity</th><th className="pb-4">Meals</th><th className="pb-4">Funded (BDT)</th><th className="pb-4">Cost (BDT)</th><th className="pb-4 text-right min-w-[120px]">Net Balance</th></tr></thead>
          ...
          <div className="p-6 md:p-8 rounded-3xl bg-white/60 backdrop-blur-2xl border border-white/80 shadow-xl">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              ...
              <select value={selectedAnalyticsUser} onChange={(e) => setSelectedAnalyticsUser(e.target.value)} className="bg-white/80 border border-slate-200 rounded-xl p-3 text-slate-800 font-bold focus:ring-2 focus:ring-blue-400 outline-none shadow-sm cursor-pointer min-w-[200px]">
                <option value="">Select a member...</option>
                {roommates.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
              </select>
            </div>
            {!activeRoommate ? (
              <div className="py-10 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl"><p className="text-slate-500 font-bold">Select a member from the dropdown to view their insights.</p></div>
            ) : (
              <div className="animate-in fade-in zoom-in-95 duration-200">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-center"><p className="text-[10px] font-bold text-slate-500 uppercase">Meals Had</p><p className="text-2xl font-black text-slate-800">{individualMeals}</p></div>
                  <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-100 text-center"><p className="text-[10px] font-bold text-emerald-600 uppercase">Contributed</p><p className="text-2xl font-black text-emerald-700">৳ {activeRoommate.spent.toFixed(2)}</p></div>
                  <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100 text-center"><p className="text-[10px] font-bold text-amber-600 uppercase">Meal Cost</p><p className="text-2xl font-black text-amber-700">৳ {(individualMeals * mealRate).toFixed(2)}</p></div>
                  <div className="p-4 rounded-2xl bg-blue-50 border border-blue-100 text-center"><p className="text-[10px] font-bold text-blue-600 uppercase">Net Balance</p><p className="text-2xl font-black text-blue-700">{activeRoommate.spent - (individualMeals * mealRate) >= 0 ? '+' : '-'} ৳ {Math.abs(activeRoommate.spent - (individualMeals * mealRate)).toFixed(2)}</p></div>
                </div>
                    ...
                    individualHistory.map((entry, idx) => (
                      <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 shadow-sm gap-3">
                        ...
                        <div className="flex flex-wrap gap-2 items-center w-full sm:w-auto">
                          ...
                          {entry.data.is_edited && <span className="text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-md bg-amber-100 text-amber-700 border border-amber-200 sm:ml-4">Edited</span>}
                        </div>
                      </div>
                    ))
                  )}
                ...
                payments.map((log) => {
                  ...
                  return (
                    <div key={log.id} className="flex justify-between items-center py-3 first:pt-0">
                      <div><p className="text-base font-bold text-slate-800">{log.description}</p><p className="text-[10px] text-slate-500 font-black uppercase tracking-wider mt-1">Posted by <span className="text-blue-600">{payer ? payer.name : 'Unknown Entity'}</span></p></div>
                      <div className="text-right"><p className="text-lg font-black text-emerald-600">+ ৳ {Number(log.amount).toFixed(2)}</p><p className="text-[10px] text-slate-400 font-bold uppercase mt-1">{new Date(log.created_at).toLocaleDateString()}</p></div>
                    </div>
                  );
                })
              )}
            ...
            <div className="flex flex-col items-center justify-center py-16 px-4 text-center rounded-3xl bg-white/60 backdrop-blur-2xl border border-white/80 shadow-xl mt-4">
              ...
              <form onSubmit={handleMagicLink} className="flex flex-col gap-3 w-full max-w-xs">
                <div className="relative">
                  ...
                  <input type="email" placeholder="Enter your email address" value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} className="bg-white/80 border border-slate-200 rounded-xl p-4 pl-12 text-slate-800 font-bold focus:ring-2 focus:ring-blue-400 outline-none shadow-inner w-full" required />
                </div>
                <button type="submit" disabled={isAuthLoading} className="bg-slate-900 hover:bg-slate-800 text-white px-8 py-4 rounded-xl font-black tracking-wide shadow-xl transition-all active:scale-[0.98] w-full disabled:opacity-50">{isAuthLoading ? 'Sending Link...' : 'Send Magic Link'}</button>
              </form>
              {authMessage && (<p className={`mt-6 text-sm font-bold p-3 rounded-lg border ${authMessage.includes('sent') ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-600 border-red-200'}`}>{authMessage}</p>)}
            </div>
          ) : (
            <>
              ...
              <div className="flex flex-col gap-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="inline-flex flex-col p-3 bg-white/40 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm">
                    ...
                    <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} disabled={role !== 'manager'} className="bg-transparent text-lg font-black text-slate-800 outline-none cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed" />
                  </div>
                </div>
              ...
              {/* NEW: ONLY MANAGERS SEE THE EXPENSE FORM */}
              {role === 'manager' && (
                <>
                  ...
                  <div className="max-w-2xl mx-auto w-full">
                    <form onSubmit={handlePostPayment} className="p-6 md:p-10 rounded-squircle bg-white/60 backdrop-blur-2xl border border-white/80 shadow-xl flex flex-col gap-6 relative overflow-hidden">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <select value={selectedUser} onChange={(e) => setSelectedUser(e.target.value)} className="bg-white/80 border border-white/40 rounded-xl p-4 text-slate-800 font-bold focus:ring-2 focus:ring-blue-400 outline-none shadow-sm cursor-pointer">
                          <option value="">Select a member...</option>
                          {roommates.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                        </select>
                      </div>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <input type="number" step="0.01" placeholder="e.g., 500.00" value={amount} onChange={(e) => setAmount(e.target.value)} className="bg-white/80 border border-white/40 rounded-xl p-4 text-slate-800 font-bold focus:ring-2 focus:ring-blue-400 outline-none shadow-inner" required />
                      </div>

                      <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Transaction Specifications</label>
                        <textarea placeholder="e.g., Rice, Chicken, Utility Bills..." rows={3} value={note} onChange={(e) => setNote(e.target.value)} className="bg-white/80 border border-white/40 rounded-xl p-4 text-slate-800 font-bold focus:ring-2 focus:ring-blue-400 outline-none shadow-inner resize-none" required />
                      </div>

                      <button type="submit" disabled={isSubmitting} className="mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-4 rounded-xl shadow-md transition-all active:scale-[0.98] disabled:opacity-50 text-lg">
