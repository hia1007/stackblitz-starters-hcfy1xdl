...
export default function Dashboard() {
  ...
  // 🔄 FIX 2: Automatically sync dropdown state when roommates load
  useEffect(() => {
    if (roommates.length > 0) {
      const isValidSelection = roommates.some(r => r.id === selectedUser);
      if (!selectedUser || !isValidSelection) {
  ...
  // 🔄 FIX 1: The comment is moved safely above the return statement so the compiler doesn't crash
  return (
