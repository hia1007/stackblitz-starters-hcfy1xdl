...
import { useAuthStore } from '../store/useAuthStore'; // NEW: Imported auth store
...
export default function MealAdjuster({ id, name, spent }: MealAdjusterProps) {
  ...
  const { role } = useAuthStore(); // NEW: Fetch role
  
  const currentMeals = dailyMeals[selectedDate]?.[id] || { ...defaultMeals };
  const isManager = role === 'manager'; // NEW: Verify authorization
  
  const today = new Date().toISOString().split('T')[0];
  const isPastDate = selectedDate < today;

  const handleToggle = (key: 'noon' | 'night' | 'hasGuest' | 'guestNoon' | 'guestNight') => {
    if (!isManager) return; // Silent rejection for non-managers
  ...
  return (
    <div className={`p-5 rounded-3xl bg-white/60 backdrop-blur-2xl border border-white/80 shadow-lg transition-all duration-300 flex flex-col gap-4 ${!isManager && 'opacity-90'}`}>
      ...
      <div className="flex flex-col gap-2">
        {options.map((opt) => {
          ...
          return (
            <label 
              key={opt.key}
              className={`flex items-center justify-between p-3 rounded-2xl border-2 select-none transition-all ${
                isManager ? 'cursor-pointer active:scale-95 hover:bg-white' : 'cursor-not-allowed opacity-80'
              } ${checked ? 'bg-blue-50 border-blue-200 text-blue-900 shadow-sm' : 'bg-white/50 border-slate-100 text-slate-500'}`}
            >
              ...
              <input 
                ...
                className="sr-only"
              />
              <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${checked ? 'bg-blue-600 border-blue-600' : 'border-slate-300'}`}>
        ...
        {currentMeals.hasGuest && (
          <div className="flex flex-col gap-2 pt-2 border-t border-slate-200 mt-1 animate-in fade-in slide-in-from-top-2 duration-200">
            {[
              ...
              <label 
                key={gOpt.key} 
                className={`flex items-center justify-between p-2.5 px-3 rounded-xl border-2 transition-all ${
                  isManager ? 'cursor-pointer active:scale-95 hover:bg-white' : 'cursor-not-allowed opacity-80'
                } ${currentMeals[gOpt.key] ? 'bg-pink-50 border-pink-200 text-pink-900' : 'bg-white/50 border-slate-100 text-slate-500'}`}
              >
                ...
                <input 
                  ...
                  className="sr-only" 
                />
                <div className={`w-4 h-4 rounded-md border-2 flex items-center justify-center transition-all ${currentMeals[gOpt.key] ? 'bg-pink-600 border-pink-600' : 'border-slate-300'}`}>
